...
- (void)setNavigationWithTitle:(NSString *)title
{
    ...
    UIView *searchView = [[UIView alloc] initWithFrame:CGRectMake(60, 25, self.view.frame.size.width - 90, 34)];
    searchView.backgroundColor = [UIColor clearColor];

    UIImageView *searchBg = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"search"]];
    searchBg.frame = CGRectMake(0, 0, 34, 34);
    [searchView addSubview:searchBg];
    ...
    //搜索框
    _searchText = [[UITextField alloc]initWithFrame:CGRectMake(34, 0, self.view.frame.size.width - 90 - 34, searchView.frame.size.height)];
    _searchText.backgroundColor = [UIColor clearColor];
    _searchText.font = [UIFont systemFontOfSize:13];
    _searchText.placeholder  = @"请输入城市名称或首字母查询";
    _searchText.returnKeyType = UIReturnKeySearch;
...
- (void)ininHeaderView
{
    _tableHeaderView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, _tableView.frame.size.width, 74)];
    _tableHeaderView.backgroundColor = [UIColor clearColor];

    //定位城市
    UILabel *title1 = [[UILabel alloc]initWithFrame:CGRectMake(10, 2, TWitdh-20, 42)];
    title1.text = [ NSString stringWithFormat:@"定位: %@",_arrayLocatingCity[0]];
    title1.font = [UIFont systemFontOfSize:15];
    title1.textColor = [UIColor colorFromHexString:@"#424242"];
    [_tableHeaderView addSubview:title1];
    
    UIView *locationView = [[UIView alloc]initWithFrame:CGRectMake(0, 15, TWitdh, 44)];
...
{
    if (_delegate) {
        [_delegate didClickedWithCityName:_arrayLocatingCity[0]];
    }
    [self dismissViewControllerAnimated:YES completion:nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell == nil) {
        ...
        [cell.textLabel setTextColor:[UIColor colorFromHexString:@"#424242"]];
